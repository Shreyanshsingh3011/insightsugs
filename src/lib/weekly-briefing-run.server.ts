...
async function generateAiSections(
  rawData: unknown,
  opts: { scope: "user" | "org"; audience: string },
): Promise<Section[]> {
...
function sectionsToMarkdown(sections: Section[], header: string): string {
  ...
  for (const s of sections) {
    ...
    if (s.bullets?.length) for (const b of s.bullets) parts.push(`- ${b}`);
    parts.push("");
  }
  return parts.join("\n");
}

export async function runWeeklyBriefing(opts: { origin: string; sendEmail?: boolean }) {
  ...
  const isAdmin = (uid: string) =>
    (rolesByUser.get(uid) ?? []).some((r) => r === "admin" || r === "super_admin");
  ...
  for (const p of prefRows ?? [])
    prefsByUser.set(p.user_id, { sections: p.sections, overdue_priority: p.overdue_priority });
  const defaultPrefs = { sections: ["projects", "sheets", "documents", "alerts"], overdue_priority: "top" };

  const briefingUrl = `${opts.origin}/briefings`;
  ...
  for (const u of users ?? []) {
    ...
    const overdueList = myActivities.filter(
      (a) => a.due_date && a.due_date < endISO && a.status !== "completed",
    );
    const sortedOverdue = [...overdueList]
      .sort((a, b) => {
        if (prefs.overdue_priority === "by_due_date")
          return (a.due_date ?? "").localeCompare(b.due_date ?? "");
        if (prefs.overdue_priority === "by_age")
    ...
    const visibleDocs = (documents ?? []).filter(
      (d) =>
        ...
        d.visibility === "public" ||
        admin_ ||
        (d.visibility === "shared" && docSharedTo.get(d.id)?.has(uid)),
    );
    const visibleSheets = (sheets ?? []).filter(
      (s) =>
        ...
        s.visibility === "public" ||
        admin_ ||
        (s.visibility === "shared" && sheetSharedTo.get(s.id)?.has(uid)),
    );
    ...
    if (enabled.has("documents")) {
      raw.documents = {
        updated_last_7d: visibleDocs.length,
        samples: visibleDocs.slice(0, 5).map((d) => d.name),
      };
    }
    if (enabled.has("sheets")) {
      raw.sheets = {
        visible: visibleSheets.length,
        samples: visibleSheets.slice(0, 5).map((s) => s.display_name),
      };
    }
    if (enabled.has("alerts")) {
      raw.alerts_and_concerns = {
        alerts_last_7d: myAlerts.map((a) => ({ activity: a.activity, severity: a.severity })),
        concerns_last_7d: myConcerns.map((c) => ({
          title: c.title,
          severity: c.severity,
          status: c.status,
        })),
    ...
    // Admins always get a briefing (org visibility); other users need some activity.
    const hasAnything =
      admin_ ||
      myActivities.length + visibleDocs.length + visibleSheets.length + myAlerts.length + myConcerns.length > 0;
    if (!hasAnything) continue;

    const sections = await generateAiSections(raw, {
      scope: "user",
      audience: u.full_name || u.email || "you",
    });
  ...
  return {
    ok: true,
    users_briefed: generated.length,
