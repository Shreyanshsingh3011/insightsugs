/**
 * Client for the hosted Emergent "studio resolve" dependency-graph service.
 * Encodes/decodes the base64 `d` query param that describes a source sheet
 * plus its configured dependency edges, and types the resolved chain
 * (topo order, transitive ancestors/descendants, skip edges) returned by
 * the resolver. See dependency-inference.ts for the local (non-hosted)
 * equivalent used when Emergent isn't configured.
 */
